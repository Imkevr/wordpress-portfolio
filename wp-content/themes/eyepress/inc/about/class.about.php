 ...
 * @since 1.0.0
 */
if ( ! class_exists( 'eyepress_About' ) ) {
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class eyepress_About {
		/**
		 ...
		 *
		 * @var eyepress_About $instance The  TI_About_Page instance.
		 */
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof eyepress_About ) ) {
				self::$instance = new eyepress_About;
		...
		public function ti_about_page_render() {
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				echo '<div class="wrap about-wrap eyepress-about-wrap">';
				...
				echo '<a href="'.esc_url('https://wpthemespace.com/product/eyepress-pro/').'" target="_blank" class="wp-badge epsilon-welcome-logo"></a>';
    ...
    return add_query_arg(
		...
		public function getting_started() {
			...
			?>
			<div class="feature-section eyepress-section eyepress-section-getting-started three-col">
				<?php foreach ( $content as $item ) : ?>
					<?php $this->render_grid_item( $item ); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .eyepress-section -->
			<?php
		}
		...
		public function support() {
			...
			?>
			<div class="feature-section eyepress-section eyepress-section-support three-col">
				<?php foreach ( $content as $item ) : ?>
					<?php $this->render_grid_item( $item ); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .eyepress-section -->
		...
		public function free_pro() {
			...
			if ( ! empty( $free_pro ) ) {
				if ( ! empty( $free_pro['free_theme_name'] ) && ! empty( $free_pro['pro_theme_name'] ) && ! empty( $free_pro['features'] ) && is_array( $free_pro['features'] ) ) {
					...
					echo '<tr><td style="border-top:0;"></td><td style="border-top:0;"></td><td style="border-top:0;" class="only-pro"><a href="'.esc_url('https://wpthemespace.com/product/eyepress-pro/').'" target="_blank" >';
					echo esc_html ('Eyepress Pro Live Preview', 'eyepress');
					echo '</a></td></tr>';
		...
		public function style_and_scripts( $hook_suffix ) {
			...
			wp_enqueue_style( 'about-page-css', get_template_directory_uri() . '/inc/about/css/about.min.css', array(),'1.6.2','all' );
			wp_enqueue_script( 'eye-notice-js', get_template_directory_uri() . '/inc/about/js/notice.js', array('jquery'), '1.6.0', true );

			if ( 'appearance_page_' . $this->theme_slug . '-welcome' == $hook_suffix ) {

				wp_enqueue_script( 'about-page-js', get_template_directory_uri() . '/inc/about/js/about.js', array( 'jquery' ) );
        ...
        'activating_string'        => __( 'Activating', 'eyepress' )
        )
    );
